...
function bullets(slide, items, y, opts = {}) {
  slide.addText(
    items.map((t, i) => ({ text: t, options: { bullet: true, breakLine: true, paraSpaceAfter: 6 } })),
...
export function buildVakilPptx() {
  const pptx = new pptxgen();
  pptx.layout = "LAYOUT_WIDE";
  pptx.author = "Vakil Case";
  pptx.title = "Vakil Case - Sales Presentation";
  pptx.subject = "Practice management platform for law firms";

  // 1 — Cover
  let s = pptx.addSlide();
  s.background = { color: DARK };
  s.addShape("rect", { x: 0, y: 0, w: 0.18, h: H, fill: { color: INDIGO } });
  s.addText("SALES ENABLEMENT DECK", { x: 1, y: 1.4, w: 10, h: 0.4, fontSize: 13, bold: true, color: "A5B4FC", charSpacing: 3 });
  s.addText("Vakil Case", { x: 1, y: 2.2, w: 11, h: 1.2, fontSize: 54, bold: true, color: "FFFFFF" });
  s.addText("The all-in-one practice management platform for modern law firms — cases, clients, billing, documents, and trust accounting in one secure workspace.", { x: 1, y: 3.6, w: 10.5, h: 1.2, fontSize: 18, color: "CBD5E1" });
  s.addText("15+ Modules   ·   100% INR Billing   ·   Secure Access Control", { x: 1, y: 5.6, w: 11, h: 0.4, fontSize: 14, bold: true, color: "818CF8" });
  ...
  const sol = [
    ...
    ["Billing & Invoices", "Generate, send, and track invoices — fully in INR with partial-payment tracking."],
    ["Documents & E-Sign", "Store, organize, and request legally-binding e-signatures on any document."],
    ["Trust Accounting", "Separate trust and operating accounts with full transaction ledgers."],
  ];
  sol.forEach((p, i) => {
  ...
  kpis(s, [["Total Cases", "48"], ["Active", "22"], ["Upcoming", "6"], ["Pending ₹", "₹4.2L"]], y);
  table(s, [
    ["Case", "Client", "Status", "Next hearing"],
    ["Sharma v. Mehta", "Rajesh Sharma", "Trial", "12 Sep"],
    ["Estate of Verma", "Anita Verma", "Discovery", "18 Sep"],
    ["Sun Corp. Merger", "Sun Industries", "Filed", "—"],
  ...
  table(s, [
    ...
    ["Rajesh Sharma", "rajesh@email.com", "+91 98...", "Active", "A. Gupta"],
    ["Anita Verma", "anita@email.com", "+91 99...", "Onboarded", "S. Rao"],
    ["Sun Industries", "legal@sun.co", "+91 80...", "Prospect", "A. Gupta"],
  ...
  table(s, [
    ...
    ["Sharma v. Mehta", "Civil Litigation", "Trial", "Urgent", "Rajesh Sharma"],
    ["Estate of Verma", "Estate Planning", "Discovery", "High", "Anita Verma"],
    ["Sun Corp. Merger", "Corporate", "Filed", "Medium", "Sun Industries"],
  ...
  // 7 — Lawyer Management
  s = pptx.addSlide();
  y = header(s, "Screen 4 · Lawyer Management", "Your team, fully credentialled", "Profiles, bar credentials, specializations, ratings, and verification status.");
  table(s, [
    ["Name", "Specialization", "Bar #", "Years", "Status"],
    ["Adv. A. Gupta", "Criminal Law", "DL/1234", "12", "Verified"],
    ["Adv. S. Rao", "Family Law", "MH/5678", "8", "Under Review"],
    ["Adv. P. Nair", "Corporate", "KL/9012", "15", "Verified"],
  ], y, [2.8, 3.0, 2.0, 1.3, 3.0]);
  bullets(s, [
    "Bar credentials — registration number and bar council on every profile.",
  ...
  // 8 — Invoices
  s = pptx.addSlide();
  y = header(s, "Screen 5 · Invoices & Billing", "Billing that gets you paid — in INR", "Create, send, and track invoices with partial payments and overdue alerts.");
  table(s, [
    ["Invoice #", "Client", "Amount", "Paid", "Status", "Due"],
    ["INV-1042", "Rajesh Sharma", inr(85000), inr(85000), "Paid", "—"],
    ["INV-1043", "Anita Verma", inr(120000), inr(60000), "Partial", "20 Sep"],
    ["INV-1044", "Sun Industries", inr(250000), inr(0), "Overdue", "01 Sep"],
  ], y, [1.8, 2.6, 2.0, 1.9, 1.8, 2.0]);
  bullets(s, [
    "Native INR billing — all invoices and trust ledgers in Indian Rupees.",
  ...
  bullets(s, [
    "Trust Accounting — separate trust and operating accounts with full deposit/withdrawal ledgers and running balances.",
  ...
  bullets(s, [
    ...
    "INR-only billing — currency compliance built in across fees, trust, and subscriptions.",
  ...
  // 12 — Why Vakil Case
  s = pptx.addSlide();
  y = header(s, "Why Vakil Case", "Built for Indian law firms, ready to scale", "Purpose-built for the way Indian advocates actually work.");
  const why = [
    ["INR-native", "Every amount — fees, trust, retainers, subscriptions — in Indian Rupees."],
    ["Jurisdiction-aware", "India-first with templates tuned for Indian courts and bar councils."],
    ...
    ["Firm-ready", "From solo advocates to multi-lawyer firms."],
  ...
  s.addText("LET'S TALK", { x: 1, y: 1.6, w: 11, h: 0.4, fontSize: 13, bold: true, color: "C7D2FE", charSpacing: 3, align: "center" });
  s.addText("See Vakil Case in action", { x: 1, y: 2.2, w: 11.3, h: 1, fontSize: 40, bold: true, color: "FFFFFF", align: "center" });
  s.addText("Book a live walkthrough and we'll tailor the demo to your practice area.", { x: 2, y: 3.4, w: 9.3, h: 0.6, fontSize: 18, color: "E0E7FF", align: "center" });
  s.addText("30-minute live demo  ·  Tailored to your practice area  ·  Free trial setup", { x: 1, y: 4.4, w: 11.3, h: 0.5, fontSize: 15, color: "C7D2FE", align: "center" });
  s.addShape("roundRect", { x: 5.16, y: 5.4, w: 3, h: 0.8, fill: { color: "FFFFFF" } });
  s.addText("Book a demo", { x: 5.16, y: 5.4, w: 3, h: 0.8, fontSize: 16, bold: true, color: INDIGO, align: "center", valign: "middle" });
